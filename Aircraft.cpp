...
{
    glm::vec3 acc(-airspeed * 0.3f + thrust * Front * 0.025f *
                                     (Position.y > 0 ? (Position.y > 500 ? 0.0f : 1.0f - Position.y / 500) : 1.0f));
...
void Aircraft::DrawHUD()
{
    ...
    ptr = 0.68f;
    bool vsneg = displayvs < 0;
    if (vsneg)displayvs = -displayvs;
